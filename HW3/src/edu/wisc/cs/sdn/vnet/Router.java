...
import edu.wisc.cs.sdn.vnet.rt.ArpCache;
...
{	
	...
	private boolean handleRIPPacket(Ethernet ether, Iface inface){ //maybe send it in this method
		...
		if (rip.getCommand() == RIPv2.COMMAND_REQUEST){
			...
			Ethernet resPacket = createRipPacket( inface, RIP_MULTICAST, ether.getSourceMACAddress(), 1); // send back a response
			...
			for (RIPv2Entry entry : entries){
				...
				RouteEntry rte = routeTable.lookup(entry.getAddress());
				if (rte != null){
					if(rte.getDistance() > distance) {
						isupdate = true;
						routeTable.update(ipAddr, subnetMask, nextHop, inface, distance, System.currentTimeMillis()); /////update(int dstIp, int maskIp, int gwIp, Iface iface, int dist, long time)
					}
				} else {
					isupdate = true;
